...
uri = "mongodb+srv://mahimagesh1313:iw3LI5hijwagxJR5@cluster0.kuyesmp.mongodb.net/?retryWrites=true&w=majority"
...
def get_staff_gate_records():
    try:
        ...
        query = {'entry_time': {'$regex': f'^{today}'}}
...
def get_student_gate_records():
    try:
        ...
        today = datetime.now().date().isoformat()
        query = {'entry_time': {'$regex': f'^{today}'}}
        records = list(gate_student_collection.find(query, {'_id': 0}))
        return jsonify(records)
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
@app.route('/api/gate/records/staff/weekly', methods=['GET'])
def get_staff_weekly_records():
    try:
        one_week_ago = datetime.now() - timedelta(days=7)
        records = list(gate_staff_collection.find(
            {"entry_time": {"$gte": one_week_ago.isoformat()}},
            {'_id': 0}
        ))
        return jsonify(records)
...
@app.route('/api/gate/records/staff/monthly', methods=['GET'])
def get_staff_monthly_records():
    try:
        one_month_ago = datetime.now() - timedelta(days=30)
        records = list(gate_staff_collection.find(
            {"entry_time": {"$gte": one_month_ago.isoformat()}},
            {'_id': 0}
        ))
        return jsonify(records)
    except Exception as e:
...
@app.route('/api/gate/records/students/weekly', methods=['GET'])
def get_student_weekly_records():
    try:
        one_week_ago = datetime.now() - timedelta(days=7)
        records = list(gate_student_collection.find(
            {"entry_time": {"$gte": one_week_ago.isoformat()}},
            {'_id': 0}
        ))
        return jsonify(records)
...
@app.route('/api/gate/records/students/monthly', methods=['GET'])
def get_student_monthly_records():
    try:
        one_month_ago = datetime.now() - timedelta(days=30)
        records = list(gate_student_collection.find(
            {"entry_time": {"$gte": one_month_ago.isoformat()}},
